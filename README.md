...
Each `dates-list` on the page is fully independent — its own filter direction, its own item count, its own template/Load More. A common pattern: two `dates-list` wraps on one page, one with `data-ix-events-dates-filter="upcoming"` and one with `"past"`.
