...
| `data-ix-events="date"` (+ `data-ix-events-date-format`, incl. `FULLDATE`) | The occurrence's date/time — same convention as List View/Feed View, see [Format attributes](#format-attributes). |
...
Within a row, multi-day occurrences are lane-assigned ahead of same-start single-day ones, so they always claim the lowest lane numbers and land at the top of every day-cell's pill stack — this is what keeps a multi-day bar connecting seamlessly across cells instead of landing underneath a single-day event that happened to grab a lower lane first. One accepted trade-off: a single-day event that starts chronologically before a multi-day event elsewhere in the same row occasionally lands in a higher lane than strict interval-packing would give it, even when the two don't actually overlap — a minor loss of lane-packing efficiency in exchange for multi-day bars never being displaced from the top.
...
- **Position**: left-aligned with the hovered pill's left edge, not centered — clamped so it never runs past the calendar's own left or right edge. Shown directly **below** the pill if the pill is in the top half of the browser viewport, or **above** it if the pill is in the bottom half (a literal viewport-relative check, not "does it fit," since that would flip inconsistently depending on the card's own height) — clamped the same way vertically so an unusually tall card still can't run off the top/bottom of the calendar. The gap between the pill and the card's final resting position is `--hover-card-gap` (default `1rem` — see [Global design tokens](#global-design-tokens)).
- **Reveal**: `is-active` toggles `opacity`/`visibility`/`pointer-events`/`transform` together, all driven by a CSS `transition` — a plain opacity fade would work, but JS also adds `is-above` or `is-below` (whichever direction the card was actually placed) so the entrance can slide `--hover-card-move` (default `0.75rem`) in that direction too: a card shown above its pill rises up into place, one shown below drops down into place. `is-above`/`is-below` are applied a frame before `is-active` (via `requestAnimationFrame`, after forcing a layout with `void card.offsetHeight`) — without that gap, the browser would collapse the "offset and hidden" starting state and the "resting and visible" end state into a single paint and skip the transition entirely, since both classes would otherwise land in the same synchronous batch. This only delays when the *reveal transition* starts, never the actual `left`/`top` positioning, which is always applied synchronously up front.
...
| `--hover-card-gap` | `1rem` | Space between a pill and its hover card — kept as its own token rather than reusing `--pill-gap`, since that one also controls the much tighter stacked-lane spacing inside a day-cell. |
