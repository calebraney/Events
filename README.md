...
**Designer setup for `fs-list-load="all"` to actually work**: in the Collection List's own settings panel, **Show** must be set to its max (**100**) — that's the count Webflow bakes into the page natively, and `fs-list-load="all"` can only paginate *past* it, never raise it. If **Show** is left lower (e.g. 9), that's a hard ceiling no Finsweet attribute can override, and items beyond it are silently never in the DOM at all (no console warning, since the script never even sees them). Control how many actually display with the separate **Paginate Items** field instead, set to whatever count you want visible per page — that one plays correctly with `fs-list-load="all"`. This is a real, previously-hit footgun: two Collection Lists left at a low **Show** value each silently dropped a different subset of events, with the shared `data-wrap` and Calendar's `hover-cards` list (both correctly set) rendering the same events perfectly fine — worth checking first any time some events "go missing" from List/Feed View but not Calendar.
...
| `data-ix-events="list"`                                  | The Collection List wrapper (same element as `fs-list-element="list"`) | No, but recommended for symmetry with Feed View | Marks the items' container explicitly. Not required for List View (Finsweet's `fs-list-element="list"` already pins that element down unambiguously), but Feed View relies on it — see §3. |
| `data-ix-events="prev"`                                  | A button, anywhere inside wrap                      | No                           | Steps back one month/week. Gets class `is-disabled` (and `aria-disabled="true"`, and does nothing when clicked) when `hide-past-events="true"` and stepping back would land entirely before today. |
| `data-ix-events="next"`                                  | A button, anywhere inside wrap                      | No                           | Steps forward one month/week.                                                                 |
| `data-ix-events="today"`                                 | A button, anywhere inside wrap                      | No                           | Jumps back to the month/week containing today's real date. Gets class `is-disabled` (and `aria-disabled="true"`, and does nothing when clicked) when the active range already contains today. |
| `data-ix-events="label"`                                 | A text element, anywhere inside wrap                | No                           | JS sets its text to the active month/week (see format options below) and gives it `aria-live="polite"` so screen readers announce the change. |
...
| `data-ix-events-item-count`          | number \| `unlimited` | `30`    | How many occurrence-cards reveal at once, chronologically across every event in the active range (not per event), with a `load-more` button revealing N more per click. The button (or its `load-more-wrap`, if present) is only shown while more remain in the active range, and the revealed count resets back to N every time the range changes (prev/next/today). Set the literal value `unlimited` to opt back into the old no-limit behavior (every occurrence in the active range shows at once, no `load-more` needed) — worth knowing this can realistically mean hundreds of cards for a page with several dense recurring events active in the same month. Shares its name and meaning with Feed View's option of the same name (whose own default stays `12` — Feed's range is unbounded going forward, so an "unlimited" feed isn't a meaningful concept the way it is for List View's fixed month/week window). |
...
| `data-ix-events="list"`                 | The Collection List wrapper       | No, but recommended  | Marks the items' container explicitly. Falls back to inferring it from the first item's own parent if absent (for instances built before this role existed), but the explicit role removes the old requirement that `feed-divider` be a sibling of the items rather than nested inside them — with `list` present, cards/dividers insert relative to this unambiguous element instead. |
...
| `data-ix-events-feed-divider-today`  | `true` \| `false`  | `false` | Only meaningful when `feed-divider` is `true`. Overrides the text of that very first divider to the literal word "Today" instead of the current month's formatted label. No effect if `feed-divider` is `false`, and no effect unless `filter="upcoming"` (a past or "all" feed's first divider is essentially never literally today). |
| `data-ix-events-filter`              | `upcoming` \| `past` \| `all` | `upcoming` | WHICH occurrences are candidates — independent of display order (see `sort` below). `upcoming` — only occurrences on/after today. `past` — only occurrences that have already ended. `all` — no time-based filtering, bounded to roughly ±3 years from today (a truly unbounded recurring series has no literal "all"). With `duplicate-recurring="false"`: `upcoming`/`all` keep the earliest occurrence in range, `past` keeps the most recent one. |
| `data-ix-events-sort`                | `earliest-first` \| `latest-first` | contextual | Display order, independent of `filter`. Unset (default) is contextual: `latest-first` when `filter="past"`, `earliest-first` otherwise — matching each filter's most natural reading order. Set explicitly to override, e.g. `filter="past"` + `sort="earliest-first"` for a chronological (not most-recent-first) history. |

**Filtering vs. sorting were previously one combined `data-ix-events-direction` option** (`"upcoming"` implied both "only future" and "soonest first"; `"past"` implied both "only past" and "most recent first") — **renamed and split**, existing instances need updating. Splitting them lets any filter pair with any sort, e.g. a "just happened" section shown oldest-first instead of most-recent-first.
...
Each click reveals the next `item-count` occurrences, searched chronologically across every event (recurring events contribute one card per occurrence in range, same as List View with `duplicate-recurring="true"`), scoped by `data-ix-events-filter` and ordered by `data-ix-events-sort`. `upcoming`/`past` search outward from today (forward/backward respectively) in expanding steps; `all` runs one bounded ±3-year query. The button (or its `load-more-wrap`, if present) hides itself once there's nothing left to reveal — including after an `upcoming`/`past` search reaches its ~3 year (month period) / ~8 month (week period) cap without finding a full batch.

### Showcase sections

To drop a small, fixed-count events section anywhere on the site (not just a dedicated events page) — e.g. "next 3 workshops" in a page footer, or a "recently wrapped" strip — build a Feed View instance with **no** `load-more`/`load-more-wrap` element and `data-ix-events-feed-divider="false"`. With no button to click, `Load More` still runs once on init and simply renders exactly `item-count` cards with no pagination UI and no dividers. Scope which events appear via Webflow's own Designer-side Collection List filter (event type, location, etc.) rather than any live/visitor-facing filter — this module only ever iterates whatever items are already in the list, so no extra filtering code or markup is involved. Add `data-ix-events-filter="past"` for a "just happened" section instead of "coming up."
...
| `data-ix-events="prev"` / `"next"` | A button, anywhere inside wrap | No | Steps back/forward one month or week (per `range`). Gets class `is-disabled` (and `aria-disabled="true"`) at the `months` bound (see Option attributes). |
| `data-ix-events="today"` | A button, anywhere inside wrap | No | Jumps to the month/week containing today's real date. Gets class `is-disabled` (and `aria-disabled="true"`) when the active range already contains today. |
| `data-ix-events="label"` | A text element, anywhere inside wrap | No | JS sets its text to the active month/week — same format vocabulary as List View's label, see [Format tokens](#format-tokens) — and gives it `aria-live="polite"` so screen readers announce the change. |
...
| `data-ix-events="grid"` | A container, anywhere inside wrap | Yes | Holds the day cells. A plain CSS Grid (`repeat(7, minmax(0,1fr))`) — row height is left to the browser's own row auto-sizing, so each week-row grows independently to fit its tallest cell with zero JS involvement. JS sets `role="grid"` and `aria-label="Event calendar"` on it — no Designer/HTML changes needed. |
| `data-ix-events="day-cell"` | 42 elements, direct structure inside grid | Yes | A fixed 6×7 grid, always. **DOM order must be row-major** — the 1st element is week-row 0/column 0, the 7th is week-row 0/column 6, the 8th is week-row 1/column 0, and so on. Build one, then duplicate it 41 times in the Designer. JS sets `role="gridcell"`, `aria-rowindex`/`aria-colindex` (1-based), and an `aria-label` combining the date and event count on each — entirely automatic. |
...
| `data-ix-events="day-more"` | A `<button>`, child of each day-cell | No | "+N more" overflow text for that specific day — JS sets its text and toggles class `is-active` only when that day has more occurrences than `day-pill-limit` allows. A real button, not a div — it's the only thing that responds to a click, never the whole cell, so pills stay independently clickable. In `overflow-items="expand"` mode, also reflects its row's expanded/collapsed state via `aria-expanded="true"/"false"`. |
| `data-ix-events="calendar-pill"` | A hidden template element, anywhere inside wrap | Yes (if any events should render) | Cloned once per **day** a segment covers (see [Rendering model](#rendering-model) below — a 3-day event gets 3 clones, one per day-cell it touches). Root element is the pill's own link (`<a>`) — its `href` is set from `data-ix-events-link-format` (see Option attributes), and it's revealed on `focus` as well as `mouseenter` (see [Hover card reveal](#hover-card-reveal)) so keyboard users get the same supplementary info a mouse hover shows. May contain any of the bindable child elements below; only include the ones you actually want visible. `is-middle`/`is-end` continuation pieces (see [Rendering model](#rendering-model)) carry no content, so they get `tabindex="-1"` + `aria-hidden="true"` — they'd otherwise be empty, indistinguishable tab stops and empty "link" announcements between one meaningful pill and the next. |
...
Within a row, lanes are assigned by start day first — a segment reuses the lowest lane whose previous occupant has already ended, regardless of which one is multi-day, so two events that don't actually overlap always end up sharing a lane rather than one being pushed into a higher lane (and needing a spacer above it) for no real reason. An earlier revision prioritized multi-day segments into the lowest lanes unconditionally across the whole row, which could push a genuinely non-overlapping single-day event elsewhere in the row into an unnecessary spacer purely because a later, unrelated multi-day event existed somewhere in the same row — a real reported bug (visible as unexplained blank space above a pill, and a taller row than the actual content needed), so that whole-row priority was removed and never reinstated.

For segments that share the same start day, though, tie-breaking follows a deliberate 3-tier order: 1) multi-day before single-day (longer-span-first among multi-day ties), 2) among single-day occurrences, ones with Show Start Time off (no visible time) before ones that show a time, 3) among the remaining timed single-day occurrences, earliest start time first. This only resolves ties between segments starting on the *same* day — it doesn't touch cross-day lane reuse, so the whole-row bug above stays fixed.
...
- **At most one card, ever**: each instance tracks whichever card is currently shown; a new `mouseenter` force-hides the previous card first (even if its own `mouseleave` was somehow missed), and leaving the grid entirely (`mouseleave` on `[data-ix-events="grid"]` itself) hides whatever's active as a backstop. The reveal itself is also guarded against a real race confirmed live: moving the mouse quickly across several pills queues one `requestAnimationFrame`-deferred `is-active` addition per hover (see the entrance-delay note above), and a card's own `mouseenter`/`mouseleave` run synchronously, so the tracked "current" card can change before an earlier one's queued frame fires — without checking that this card is *still* the tracked one when that frame runs, its `is-active` would get forced back on after its own `mouseleave` already removed it, leaving it stuck visible. Together these make "more than one card visible" — or one stuck open — structurally hard to hit, not just unlikely.
- **Keyboard access**: every pill also reveals/dismisses its card on `focus`/`blur`, identical logic to `mouseenter`/`mouseleave` — a keyboard user tabbing through the calendar gets the same supplementary info a mouse hover shows. `focus`/`blur` fire synchronously and always in the correct order (unlike mouse events, which can be missed during fast pointer movement), so no separate keyboard backstop is needed the way the mouse path has one.

#### Reduced motion

The reveal/dismiss transition includes a directional slide (`transform: translateY(...)`, driven by `--hover-card-move`) as well as an opacity fade. The slide is the kind of motion `prefers-reduced-motion` exists to let a user opt out of; the fade alone is generally fine to keep. Add this to the same embed as the rest of the calendar's CSS to disable just the slide for users who've requested reduced motion, leaving the fade untouched:

```css
@media (prefers-reduced-motion: reduce) {
  .calendar_hover_card_wrap,
  .calendar_hover_card_wrap.is-above,
  .calendar_hover_card_wrap.is-below,
  .calendar_hover_card_wrap.is-active {
    transform: none;
  }
}
```

(Adjust the class name if your build doesn't use `calendar_hover_card_wrap` — it's whatever class `[data-ix-events="hover-card"]` carries in your instance, per `design/calendar-mockup.html`.)
...
| Attribute                        | Values                          | Default    | Purpose                                                                                                                                                                                                    |
| ---------------------------------- | -------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `data-ix-events-filter`            | `upcoming` \| `past` \| `all`    | `upcoming` | WHICH occurrences are candidates — independent of display order (see `sort` below). `upcoming` — only occurrences on/after today. `past` — only occurrences that have already ended. `all` — no time-based filtering, bounded to roughly ±3 years from today (or the event's own Start Date / Recurring End Date, if narrower) — a truly indefinite recurring series has no literal "all." |
| `data-ix-events-sort`              | `earliest-first` \| `latest-first` | contextual | Display order, independent of `filter`. Unset (default) is contextual: `latest-first` when `filter="past"`, `earliest-first` otherwise. Set explicitly to override, e.g. `filter="all"` + `sort="latest-first"` for the full history newest-first instead of oldest-first. |
...
**Renamed from `data-ix-events-dates-filter`, and split into two independent options** (`filter` + `sort`) — existing instances need updating. Shares this filter/sort split with Feed View (§3) — see that section for the "why split" rationale.
...
- Every view marks each wrap it processes with `data-ix-events-initialized` and skips any wrap already carrying that attribute. This matters if the site uses a page-transition script that re-runs page scripts on soft navigation (no full page reload) — without this guard, a second pass through the same DOM would re-append/re-render on top of what's already there (most visibly: a Feed instance would render more cards than its `item-count`, since it only ever appends and each pass starts its own count fresh, unaware of a prior pass).
- `eventList()`, `eventFeed()`, `eventDetail()`, and `calendar()` are each called inside their own `try`/`catch` in `index.js` — an uncaught exception in one (e.g. from unexpectedly malformed page content) can't prevent the other three from running on that page.

### Timezone handling

Every date/time in this system — recurrence math, "is this occurrence upcoming or past," and the displayed hour/minute — is computed using the **visitor's own browser-local time**, not a fixed timezone. The **Timezone** field is parsed and available (e.g. it drives the abbreviation `TIME`/`TIME-SHORT` can append — see [`TIME` / `TIME-SHORT`](#time--time-short-date-element-only)), but it never adjusts the underlying date math. Practically: for a single-region audience this is invisible, since everyone's browser is in roughly the same zone as the events themselves. For a geographically distributed audience, or an event whose time is meant to be fixed regardless of who's viewing it (e.g. a webinar genuinely happening at 2pm Eastern), a visitor in a different zone will see the same wall-clock numbers ("2:00pm") the event was authored with in Webflow — not a time converted for their own zone. Full timezone-aware date math (actually shifting the displayed time per visitor) isn't implemented; if a project needs it, that's a larger, separate piece of work, not something to assume is already handled.
