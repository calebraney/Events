...
**Nesting another instance inside this one is safe** — e.g. put the whole template page's `main` in a `[data-ix-events="wrap"][data-ix-events-layout="detail"]`, then drop an "Other Upcoming Events" Feed View section (its own `[data-ix-events="wrap"][data-ix-events-layout="feed"]`) inside it. Every element this module looks for only matches something whose *nearest* ancestor of the relevant kind — the detail wrap for `data`/`next-occurrence`/`occurrence-list`, or a specific `occurrence-list` for that list's own `occurrence-item`/`load-more`/`load-more-wrap` — is the container actually being searched. So a nested Feed section's own JSON/cards/Load More button, or a *different* occurrence-list's own template/button, is never mistaken for another's.
...
| `data-ix-events="load-more-wrap"` / `"load-more"`      | Inside an `occurrence-list`               | No       | Reveals the next batch of that list's occurrence rows.                                                                                                                                     |

### Option attributes (read from each `occurrence-list` element — NOT the wrap)
...
Each `occurrence-list` on the page is fully independent — its own filter direction, its own item count, its own template/Load More. A common pattern: two `occurrence-list` wraps on one page, one with `data-ix-events-detail-filter="upcoming"` and one with `"past"`.
