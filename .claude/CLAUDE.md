...
A pill's own `[data-ix-events="date"]` is unconditionally a time-only display, not a date — `createPill()` overrides whatever `data-ix-events-date-format` is authored on it (`formatPillTime()` in `date-utils.js`, no format string needed at all) rather than leaving it to the generic `setDateFields()` path List/Feed/hover-card all use. Hidden (`display: none`) whenever that event's own `showStartTime` is off, regardless of anything else; otherwise a single start time, promoted to a start-end range (`"8:00-9:30pm"`/`"8:00am-5:00pm"`) only when **both** the new wrap-level `data-ix-events-show-end-time` option and that specific event's own `showEndTime` are true (and the occurrence has a genuine duration — `end !== start`). `formatPillTime()` shares its meridiem-collapsing rule (`hideStartMeridiem()`, extracted from what was inline logic in `formatFullDate()`) with `FULLDATE`'s own time-range formatting, but deliberately uses the padded `"h:mma"` token format for each side instead of `formatClockTime()`'s zero-hiding — matching the pill's pre-existing single-time convention (`"6:00pm"`, not `"6pm"`) rather than `FULLDATE`'s prose convention.
