...
### `event-data.js` — `whenEvents(callback)`

The single source of truth for "what events exist on this page." Both `event-list.js` and `calendar.js` call this and only this to get event data — there is no second code path that reads the JSON differently.

What it does, in order:
1. Look for `[data-ix-events="data-wrap"]` (the Collection List that holds one hidden `<script type="application/json">` per CMS item). If more than one exists on the page, warns and uses the first — this is a deliberate constraint: one page, one data source, so a List View and a Calendar on the same page can never read two different datasets.
2. If it's not in the DOM yet, retry every 300ms, up to 20 times (6 seconds), then give up and call back with `[]`. This is the one "waiting for the page to catch up" spot outside of Finsweet.
3. Once found: if that data-wrap carries `fs-list-element="list"` (meaning Finsweet needs to paginate past Webflow's ~100-item cap), queue a callback via `window.FinsweetAttributes.push(['list', ...])` and wait on that List instance's `loadingPaginatedItems` promise before reading anything. If it doesn't carry that attribute, skip straight to parsing.
...
This function is called independently by each layout that needs it — there's no caching or single shared promise. If a page has a List View, a Feed View, and a Calendar all at once, `whenEvents` runs (and re-scans/re-parses) three separate times. Cheap in practice (it's just DOM queries + `JSON.parse` on however many items exist), but worth knowing if you're ever chasing a performance question.
...
- **`buildEntries(wrap, eventsBySlug)`** — pairs each `[data-ix-events="item"]` with its event, either by parsing the item's own embedded JSON ("combined" mode) or by looking up its `data-ix-events-slug` against the page-wide `eventsBySlug` map ("separate" mode, for when the visible cards live in a different Collection List than the data).
...
- **`initCalendar(wrap)`** — one-time setup per instance: reads options, builds the hover-card slug map, wires nav button listeners, calls `refresh()` once immediately (renders an empty but structurally-correct grid before data even arrives), then calls `whenEvents()` to get real data.
...
5. Every layout that found at least one wrap calls whenEvents(callback)
   — this is the FIRST real waiting point:
     a. Look for [data-ix-events="data-wrap"]. If missing, retry every
        300ms, up to 6 seconds total, then give up with [].
     b. If found and it's marked for Finsweet pagination
...
6. calendar()'s whenEvents callback fires:
...
7. eventList()'s whenEvents callback fires (separately from Calendar's —
   remember, each layout calls whenEvents independently):
     - builds eventsBySlug map
     - for EACH list-layout wrap, calls whenListReady(wrap, required: true,
       ...) — the SECOND waiting point, specific to List View: resolving
       Finsweet's own List module and finding the instance matching this
       wrap's fs-list-element="list", via the same
       window.FinsweetAttributes queue mechanism. Also re-awaits
       loadingPaginatedItems here (independently of step 5b — the DATA
       source and the CARD source can be two different, independently
       paginated Collection Lists in "separate" mode).
...
8. eventFeed()'s whenEvents callback fires (also separately):
     - for each feed-layout wrap, whenListReady(wrap, required: false, ...)
       — optional wait, only relevant if fs-list-load="all" is set on that
       specific feed's Collection List; proceeds immediately otherwise.
...
- **Data source** — the exact same `whenEvents()` call, same JSON schema, same `parseEventFromJSON`.
...
- Options unique to it: `feed-count`, `feed-period`, `feed-divider`, `feed-divider-today`.
