...
Indirect, in two places. For its event *data*, `calendar.js` calls `whenEvents(wrap, ...)` like everyone else, and that function's own internal Finsweet-pagination-await (step 5b above) is invisible to the caller — Calendar just gets a plain array back, whether or not Finsweet was involved in producing it.

For its optional *hover cards*, `calendar.js` has its own small, direct Finsweet dependency: `resolveHoverCards()` checks whether the wrap contains a `[fs-list-element="list"]` Collection List (the same attribute `event-list.js` looks for) and, if so, queues onto `window.FinsweetAttributes` and awaits that list instance's `loadingPaginatedItems` before scanning for `[data-ix-events="hover-card"]` elements — otherwise it builds the map on a deferred microtask (`queueMicrotask`, not called inline — see below). This exists because a hover-cards Collection List reusing Finsweet pagination (`fs-list-load="all"`, or even just a Designer "Items per page" cap under the true event count) would otherwise only have its *first page* of cards in the DOM at the moment Calendar's synchronous init ran, silently dropping hover cards for any event past that first page — the same class of bug `event-data.js`'s wait already guards against for the main data source, just independently, since hover-cards and the main data-wrap are typically two different Collection Lists.

This introduced a second, subtler bug the first time it was written, worth recording: pills are created by `renderGrid()`, which can run *before* `resolveHoverCards()` finishes (e.g. `whenEvents()`'s own, separate async data load resolves first and triggers a render). Each pill's mouseenter/mouseleave listeners originally captured `hoverCardsBySlug.get(slug)` **once, at pill-creation time**, closing over that value — so any pill built during that race window permanently baked in `card = undefined`, even after `hoverCardsBySlug` was later fully populated. The map itself was correct; the *pills* were reading a stale snapshot of it from before it was ready. Fixed by moving the lookup *inside* each listener (`hoverCardsBySlug.get(seg.event.slug)` runs fresh on every actual hover, not once at render time) — this makes hover correctness independent of render/resolution ordering entirely. `resolveHoverCards()`'s callback also now calls `refresh()` once resolved, purely so the render pass's one-time "unmatched slugs" diagnostic warning reflects final data rather than a possibly-incomplete earlier pass — the lazy lookup is what makes hovering itself actually work, not this re-render.
